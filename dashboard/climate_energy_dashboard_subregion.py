...
selected_subregions = st.sidebar.multiselect(
    ...
    key="select_subregions_multiselect"
)

st.sidebar.markdown("---")
df_year = df[df['year'] == selected_year]
df_filtered = df_year[df_year['subregion'].isin(selected_subregions)]
...
if df_filtered.empty:
    st.warning("No data available for the selected filters. Please adjust year or subregions.")
    st.stop()
...
with st.expander("ℹ Filter Guide 👈"):
    st.markdown("""
    - **Year** and **Subregion(s)** filters apply to Visuals **1-4, 6, and 7**
...
yearly = df[df['subregion'].isin(selected_subregions)].groupby('year').agg({
...
fig_trend = px.line(
    yearly_long, x='year', y='Value', color='Indicator', facet_col='Indicator', facet_col_wrap=1,
    color_discrete_map={
        'renewables_share_pct': '#1f77b4',
        'co2_per_capita_t': '#ff7f0e',
        'elec_access_pct': '#2ca02c',
    },
    labels={'year': 'Year', 'Value': ''}, title='Global Trends (2000–2020)'
)
...
# --- Visual 2: Country Leaders in Climate Action ---
st.markdown("---")
...
with col1:
    ...
    fig1 = px.bar(top_renew, x='renewables_share_pct', y='country', orientation='h', color='subregion',
...
with col2:
    ...
    top_reducers = top_reducers.merge(df[['country', 'subregion']].drop_duplicates(), on='country', how='left')
    fig2 = px.bar(top_reducers.dropna(subset=['subregion']), x='change', y='country', orientation='h', color='subregion',
                  title=f'Top 10 CO₂ Reducers ({min(df["year"])}–{max(df["year"])})')
    fig2.update_layout(yaxis=dict(autorange="reversed"))
    st.plotly_chart(fig2, use_container_width=True)

# --- Visual 3: Emissions vs Efficiency Explorer ---
st.markdown("---")
...
# --- Visual 4: Regional Momentum in Renewables Share (5-Year Change) ---
st.markdown("---")
st.markdown("### 4. Regional Momentum in Renewables Share (5-Year Change)")
momentum = df.groupby(['subregion', 'year'])[selected_metric].mean().reset_index()
momentum_pivot = momentum.pivot(index='subregion', columns='year', values=selected_metric).diff(axis=1).fillna(0)
...
# --- Visual 5: CO₂ per Capita vs Energy Intensity (Quadrant) ---
st.markdown("---")
...
fig_quad = px.scatter(
    ...
    title='CO₂ per Capita vs Energy Intensity by Subregion'
)
fig_quad.add_shape(
    type='line',
...
# --- Visual 6: CO₂ Distribution Over Time ---
st.markdown("---")
st.markdown("### 6. CO₂ Distribution Over Time")

df_box = df[df['subregion'].isin(selected_subregions)].copy()
...
fig_box = px.box(
    ...
    title='CO₂ per Capita Distribution by Subregion Over Time'
)
st.plotly_chart(fig_box, use_container_width=True)

# --- Visual 7: Energy Mix Transition by Subregion ---
st.markdown("---")
st.markdown("### 7. Energy Mix Transition by Subregion")

mix = df_year[df_year['subregion'].isin(selected_subregions)] \
    .groupby('subregion')[['fossil_elec_twh', 'renew_elec_twh', 'nuclear_elec_twh']].sum().reset_index()

if not mix.empty:
    labels = list(mix['subregion']) + ['Fossil', 'Renewables']
...
# --- Visual 8: Top Countries by 5-Year Gain in Renewables ---
st.markdown("---")
...
# --- Visual 9: Global View: Energy & Emissions Landscape ---
st.markdown("---")
